...
export default function KanbanBoard() {
  ...
  const [newTask, setNewTask] = useState({
    ...
    description: '',
    status: 'todo',
    category: 'executives', // Change default category to executives
    assignee: '',
    for: '',
    dueDate: '',
    priority: 'medium'
  ...
  const handleAddTask = () => {
    ...
    if (newTask.category === 'branchmanagers') {
      setData({
        ...data,
        tasks: [...data.tasks, newTaskWithId],
        branchManagerIssues: [...data.branchManagerIssues, newTaskWithId.title]
      });
    } else if (newTask.category === 'ongoingprojects') {
      setData({
        ...data,
        tasks: [...data.tasks, {...newTaskWithId, status: 'inprogress', category: 'projects'}]
      });
    } else if (newTask.category === 'burningissues') {
      setData({
        ...data,
        tasks: [...data.tasks, newTaskWithId],
        burningIssues: [...data.burningIssues, newTaskWithId.title]
      });
    } else if (['executives', 'regionmanagers'].includes(newTask.category)) {
      setData({
        ...data,
        tasks: [...data.tasks, newTaskWithId]
      });
    } else {
      // Handle regular tasks (To Do, In Progress, Done)
      setData({
        ...data,
        tasks: [...data.tasks, newTaskWithId]
      });
  ...
  const handleUpdateTask = () => {
    if (!editingTask) return; // Ensure editingTask is not null
    const updatedTasks = data.tasks.map(task => 
  ...
  const handleAddTeamMember = () => {
    if (newTeamMember.trim()) {
      const newMember = {
        id: `tm${data.teamMembers.length + 1}`,
        name: newTeamMember.trim(),
        color: 'from-gray-500 to-gray-700' // Default color
      };
      setData({
        ...data,
  ...
  const handleAddBurningIssue = () => {
    if (newBurningIssue.trim()) {
      setData({
        ...data,
        burningIssues: [...data.burningIssues, newBurningIssue.trim()]
      });
      setNewBurningIssue('');
    }
  };
  
  const handleDeleteBurningIssue = (index) => {
    const updatedIssues = [...data.burningIssues];
    updatedIssues.splice(index, 1);
    setData({
      ...data,
      burningIssues: updatedIssues
    });
  };
  ...
  const handleAddSupportItem = () => {
    if (newSupportItem.trim()) {
      setData({
        ...data,
        supportItems: [...data.supportItems, newSupportItem.trim()]
      });
      setNewSupportItem('');
    }
  };

  const handleDeleteSupportItem = (index) => {
    const updatedItems = [...data.supportItems];
    updatedItems.splice(index, 1);
    setData({
      ...data,
      supportItems: updatedItems
    });
  };
  ...
  const handleCategoryChange = (e) => {
    ...
    setNewTask({
      ...
      status: newCategory === 'ongoingprojects' ? 'inprogress' : newTask.status
    });
  };
            ...
            <div className="flex h-full">
              ...
              <div className="flex gap-4 overflow-x-auto">
                  ...
                  >
                    ...
                    <div className="p-3 border-t border-gray-700">
                      <button 
                        ...
                        onClick={() => {setNewTask({...newTask, status: column.id}); setShowAddTask(true);}}
                      >
                ...
                <div className="flex-shrink-0 w-80">
                  <div className={`rounded-lg ${darkMode ? 'bg-gradient-to-br from-rose-900 via-rose-800 to-gray-900' : 'bg-rose-50'} border border-rose-600 shadow-lg h-full flex flex-col`}>
                    ...
                    <div className="p-3 border-t border-rose-600">
                      <button 
                        ...
                        onClick={() => {setNewTask({...newTask, category: 'projects', status: 'inprogress'}); setShowAddTask(true);}}
                      >
                        ...
                        {data.burningIssues.map((issue, index) => (
                          <div key={index} className={`p-3 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} flex justify-between items-center`}>
                            ...
                            <button 
                              ...
                              onClick={() => handleDeleteBurningIssue(index)}
                            >
                              <X size={16} />
                            </button>
                          </div>
                  ...
                  data.branchManagerIssues.map((issue, index) => (
                    <div key={index} className={`p-3 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} flex justify-between items-center`}>
                      ...
                      <button 
                        ...
                        onClick={() => handleDeleteBranchManagerIssue(index)}
                      >
                        <X size={16} />
                      </button>
                    </div>
                  ...
                  data.supportItems.map((item, index) => (
                    <div key={index} className={`p-3 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} flex justify-between items-center`}>
                      ...
                      <button 
                        ...
                        onClick={() => handleDeleteSupportItem(index)}
                      >
                        <X size={16} />
                      </button>
                    </div>
                      ...
                      <div className="relative">
                        ...
                        >
                          <optgroup label="Types">
                            ...
                            <option value="ongoingprojects">Ongoing Projects</option>
                          </optgroup>
                        </select>
                        <ChevronDown className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={16} />
                      </div>
